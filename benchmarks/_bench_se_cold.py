"""Cold-start, end-to-end, single-process SE benchmark.

- flat_start=True
- end-to-end timing from estimator construction to estimate() finish
- supports AC / DC / Hybrid
"""
...
CASE_MAP = {
    "ieee300":      ("ac", PROJECT_ROOT / "data" / "model" / "ac" / "ieee300.e", PROJECT_ROOT / "data" / "meas" / "ac" / "ieee300.meas"),
    "ieee3k":       ("ac", PROJECT_ROOT / "data" / "model" / "ac" / "ieee3k.e", PROJECT_ROOT / "data" / "meas" / "ac" / "ieee3k.meas"),
    "ieee1w":       ("ac", PROJECT_ROOT / "data" / "model" / "ac" / "ieee1w.e", PROJECT_ROOT / "data" / "meas" / "ac" / "ieee1w.meas"),
    "ieee3w":       ("ac", PROJECT_ROOT / "data" / "model" / "ac" / "ieee3w.e", PROJECT_ROOT / "data" / "meas" / "ac" / "ieee3w.meas"),
    "dc_net_1000":  ("dc", PROJECT_ROOT / "data" / "model" / "dc" / "dc_net_1000.e", PROJECT_ROOT / "data" / "meas" / "dc" / "dc_net_1000.meas"),
    "dc_net_3000":  ("dc", PROJECT_ROOT / "data" / "model" / "dc" / "dc_net_3000.e", PROJECT_ROOT / "data" / "meas" / "dc" / "dc_net_3000.meas"),
    "dc_net_3w":    ("dc", PROJECT_ROOT / "data" / "model" / "dc" / "dc_net_3w.e", PROJECT_ROOT / "data" / "meas" / "dc" / "dc_net_3w.meas"),
    "qinling_100":  ("hybrid", PROJECT_ROOT / "data" / "model" / "hybrid" / "qinling_100.e", PROJECT_ROOT / "data" / "meas" / "hybrid" / "qinling_100.meas"),
    "qinling_1000": ("hybrid", PROJECT_ROOT / "data" / "model" / "hybrid" / "qinling_1000.e", PROJECT_ROOT / "data" / "meas" / "hybrid" / "qinling_1000.meas"),
    "hybrid_net_4k": ("hybrid", PROJECT_ROOT / "data" / "model" / "hybrid" / "hybrid_net_4k.e", PROJECT_ROOT / "data" / "meas" / "hybrid" / "hybrid_net_4k.meas"),
    "hybrid_net_4w": ("hybrid", PROJECT_ROOT / "data" / "model" / "hybrid" / "hybrid_net_4w.e", PROJECT_ROOT / "data" / "meas" / "hybrid" / "hybrid_net_4w.meas"),
}
...
def run_case(case_name: str):
    ...
    result = _silent(est.estimate, verbose=False)
    ...
    return {
        ...
        'observable': bool(result.observability.observable),
        'state_count': int(result.observability.state_count),
        'measurement_count': int(result.observability.measurement_count),
    }
...
def main():
    parser = argparse.ArgumentParser(description='Cold-start SE benchmark (single-process, end-to-end).')
    ...
    print(f'Measure: {args.repeat} cold-start rounds...')
    all_runs = []
    for round_idx in range(args.repeat):
        print(f'round {round_idx + 1}: ' + ' -> '.join(ordered_cases))
        for case in ordered_cases:
    ...
    if args.out:
        with open(args.out, 'w', encoding='utf-8') as f:
            json.dump({'repeat': args.repeat, 'summary': summary, 'runs': all_runs}, f, indent=2, ensure_ascii=False)
        print(f'\nWrote {args.out}')
