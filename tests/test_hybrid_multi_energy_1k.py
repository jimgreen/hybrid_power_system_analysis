...
from scripts.generate_hybrid_multi_energy_1k import COUPLING_TYPES, NODE_COUNTS
from secore.hybrid_se import HybridStateEstimator


CASE = ROOT / "data" / "model" / "hybrid" / "hybrid_multi_energy_1k.e"
MEASUREMENTS = ROOT / "data" / "meas" / "hybrid" / "hybrid_multi_energy_1k.meas"


def test_multi_energy_1k_structure_and_control_coverage():
    book = EBook(CASE)
    node_blocks = {
        "ac": "ACNode",
        "dc": "DCNode",
    ...
    assert sum(actual_counts.values()) == 1000
    assert all(len(book.data[table_name].data) == 5 for table_name in COUPLING_TYPES)
    ...
    assert len(calc.energy_coupling_plans) == 55
...
def test_multi_energy_1k_lf_is_one_global_newton_problem(monkeypatch):
    ...
    assert calc.total_vars == calc.total_eq == 1743
    ...
    assert jacobian.shape == (1743, 1743)
    assert jacobian.nnz > 6000
...
def test_multi_energy_1k_se_is_observable_accurate_and_joint(monkeypatch):
    ...
    assert observability.rank == observability.state_count == 1805
    ...
    assert result.H.shape == (5432, 1805)
