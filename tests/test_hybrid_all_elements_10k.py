...
CASE = ROOT / "data" / "model" / "hybrid" / "hybrid_all_elements_10k.e"
MEASUREMENTS = ROOT / "data" / "meas" / "hybrid" / "hybrid_all_elements_10k.meas"


class HybridAllElements10KTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.case_book = EBook(CASE)
        cls.measurement_rows = EBook(MEASUREMENTS).data["Measurement"].data

    def test_every_required_element_family_has_at_least_twenty_devices(self):
        minimum_twenty = (
            "ACBranch",
            "DCBranch",
            "ACZeroBranch",
        ...
        for block_name in minimum_twenty:
            with self.subTest(block=block_name):
                self.assertGreaterEqual(len(self.case_book.data[block_name].data), 20)
        self.assertEqual(5000, len(self.case_book.data["ACNode"].data))
        self.assertEqual(5000, len(self.case_book.data["DCNode"].data))

        dcac_rows = self.case_book.data["DCACConverter"].data
        labels = {label: sum(row["dev_type"] == label for row in dcac_rows) for label in {row["dev_type"] for row in dcac_rows}}
        self.assertEqual({"ACDCConverter": 20, "DCACConverter": 20}, labels)

    def test_control_modes_and_switching_states_are_complete(self):
    ...
    def test_hybrid_lf_converges_from_flat_start(self):
        ...
        self.assertEqual(12, calc.iterations)
        ...
        for result, keys in (
            ...
            for key in keys:
                with self.subTest(result=key):
                    ...
                    self.assertGreaterEqual(values.shape[0], 20)
